...
export function generatePeriod(gameRepresentationId) {
  ...
  let nthMinuteSinceDayStart = getNthMinuteSinceDayStart();
  let actualminutes = 0;
  if (gameRepresentationId == "02") {
    actualminutes = Math.floor(nthMinuteSinceDayStart / 3) + 1;
  } else if (gameRepresentationId == "03") {
    actualminutes = Math.floor(nthMinuteSinceDayStart / 5) + 1;
  } else {
    actualminutes = nthMinuteSinceDayStart + 1;
  }
...
export function getNthSecondSinceDayStart() {
  const now = moment();
  const startOfDay = moment().startOf('day');
  return now.diff(startOfDay, 'seconds'); // Get difference in seconds
}

export function generateHalfMinutePeriod(gameRepresentationId) {
  const TodayPeriod = moment().format("YYYYMMDD");
  const totalSecondsSinceDayStart = getNthSecondSinceDayStart();
  const nthHalfMinuteSinceDayStart = Math.floor(totalSecondsSinceDayStart / 30) + 1;
  const paddedNthHalfMinute = _.padStart(
    nthHalfMinuteSinceDayStart.toString(),
    4,
    "0",
  );
  let NewGamePeriod = `${TodayPeriod}${gameRepresentationId}${paddedNthHalfMinute}`;
  return parseInt(NewGamePeriod);
}
