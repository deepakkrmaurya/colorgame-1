...
function injectCss() {
    ...
    style.appendChild(document.createTextNode(`
       .listContainer{
           ...
           display:flex;
           height:70px;
           border-bottom: 0.02667rem solid #171b3f;
       }
...
function GetMyEmerdList(datas) {
    ...
    let html = datas.map((data, index) => {
        ...
        <div class="order-item" data-v-03b808c2="" id="${index}">
            <div data-v-b4b99df8="" class=" listContainer">
                ...
                ${data.status !== 0
                    ? `<div data-v-b4b99df8="" class="MyGameRecordList__C-item-r ${data.status == 1 ? "success" : ""}">
                        <div data-v-b4b99df8="" class="${data.status == 1 ? "resultContainer success" : "resultContainer failed"}">${data.status == 1 ? "Success" : "Failure"}</div>
                     </div>`
                    : ""
                }
                <div class="money ${data.status === 0 ? 'd-none' : ''}">
                    <span class="${data.status === 1 ? 'success' : 'failed'}">${(data.status == 1) ? '+' : '-'}${(data.status == 1) ? data.get : data.price}.00</span>
                </div>
            </div>
            <div class="expanded-content order-details">
                ${generateOrderDetails(data)}
            </div>
        </div>
           
        `;
    }); // Ensure to join the array into a single HTML string
