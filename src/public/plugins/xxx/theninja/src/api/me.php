<?php
include "./conn.php";
//$connect = new PDO("mysql:host=173.201.253.19;dbname=red", "red", "red");
...
if ($action == 'getuserinfo') {
	$user = $_GET['user'];
	$sql = "SELECT id,nickname,phone,usercode,ROUND(money) AS money,ROUND(bonus) AS bonus FROM `users` WHERE phone='$user'";
	...
	$opt9t = "SELECT COUNT(*) as total9 FROM `signin` WHERE phone='$user' AND DATE(`created`) =date(now()-interval 12 hour)";
