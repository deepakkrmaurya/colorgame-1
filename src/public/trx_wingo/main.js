...
const alertMessage = (text) => {
    ...
    msg.className = "message_alert_text";
    ...
    setTimeout(() => {
        overlay.classList.add("v-enter-active", "v-enter-to");
    }, 0); // Wait for the next frame

    setTimeout(() => {
        overlay.classList.remove("v-enter-active", "v-enter-to");
        overlay.classList.add("v-leave-active", "v-leave-to");

        setTimeout(() => {
            document.body.removeChild(overlay);
        }, 300); // Time to wait before removing
    }, 2000); // Duration before starting leave animation
};
