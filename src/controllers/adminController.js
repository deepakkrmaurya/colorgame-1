...
const settingOTH = async (req, res) => {
    ...
    if (typer == 'becone') {
        await connection.query(`UPDATE beconeperiod SET nxt = ? WHERE id = '1'`, [info]);
        return res.status(200).json({
            message: 'Done!',
            status: true,
        });
    ...
    if (typer == 'ab') {
        await connection.query(`UPDATE abperiod SET nxt = ? WHERE id = '1'`, [info]);
        return res.status(200).json({
            message: 'Done!',
            status: true,
        });
    ...
    if (typer == 'vip') {
        await connection.query(`UPDATE vipperiod SET nxt = ? WHERE id = '1'`, [info]);
        return res.status(200).json({
            message: 'Done!',
            status: true,
        });
    ...
    if (typer == 'wheel') {
        await connection.query(`UPDATE wheelperiod SET nxt = ? WHERE id = '1'`, [info]);
        return res.status(200).json({
            message: 'Done!',
            status: true,
        });
