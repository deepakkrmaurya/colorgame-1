...
const login = async (req, res) => {
  ...
  function timerJoin(params = '', timezone = 'Asia/Kolkata', delayHours = 0, delayMinutes = 0) {
    let date = '';
    if (params) {
  ...
  let checkTime = timerJoin(time, 'Asia/Kolkata', 5, 30);
  try {
    ...
    const sql_login = "INSERT INTO login SET phone = ?, today= ?";
    await connection.execute(sql_login, [phoneNumber, checkTime]);
    await connection.execute(
      "UPDATE users SET token = ?,login_at= ? WHERE phone = ? ", [md5(accessToken), checkTime, phoneNumber],
    );
    return res.status(200).json({
