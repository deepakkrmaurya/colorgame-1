...
const initiateUPIPayment = async (req, res) => {
  ...
  try {
   ...
   const formData = {
      ...
      user_token: "b0405dcdb111daa29a8c68b3886d95ab",
      amount: "1",
      order_id: orderId,
      redirect_url: "https://20lottery.in/mian",
      remark1: "Payment Initiated",
      remark2: "Payment Initiated2"
    };
    
    // Function to create URL-encoded string
    ...
    const newRecharge = {
      ...
      status: 0,
      today: rechargeTable.getCurrentTimeForTodayField(),
      url: ekqrData.result.payment_url,
      time: rechargeTable.getCurrentTimeForTimeField(),
    };
    ...
    return res.status(200).json({
      ...
      recharge: recharge,
      urls: {
        web_url: ekqrData.result.payment_url,
      },
      status: true,
      timeStamp: timeNow,
    });
...
const verifyOnlinePayment = async (req, res) => {
    try{
        console.log("Mpay callback: ", req.body); // Log the entire request body
    let orderId = req.body.order_id; // Extract the order_id field
    let statusp = req.body.status;   // Extract the status field
    let remark1 = req.body.remark1;  // Extract the remark1 field
    const [info] = await connection.query(`SELECT * FROM recharge WHERE id_order = ? ORDER BY id DESC LIMIT 1`,[orderId]);
    if (statusp === 'SUCCESS') {
    await connection.query(`UPDATE recharge SET status = 1 WHERE id = ?`, [info[0].id]);
    await connection.query(
      `UPDATE users SET money = money + ?, total_money = total_money + ? WHERE phone = ?`,
      [info[0].money, info[0].money, info[0].phone]  // Corrected parameters
    );
    }
    else{
         await connection.query(`UPDATE recharge SET status = 2 WHERE id = ?`, [info[0].id])
    }
    return res.status(200).json({"status":'success'});
    }catch(e){
        console.log(e);
    return res.status(200).json({"status":"fails"});
    }
}
