...
const aviator = async (req, res) => {
  let auth = req.cookies.auth;
  res.redirect(
    `https://jetx.lalabetix.com/theninja/src/api/userapi.php?action=loginandregisterbyauth&token=${auth}`,
  );
};
  ...
  const queries = {
    Bets: {
      query: `SELECT id_product AS id, (money + fee) AS money, 'negative' AS type, 'Bet' AS name, status, time 
              FROM minutes_1 
              WHERE phone = ? AND time >= ? AND time <= ? AND (status = 1 OR status = 2)`,
    ...
    // },
    Recharge: {
      query: `SELECT id_order AS id, money, 'positive' AS type, 'Recharge' AS name, status, time FROM recharge WHERE phone = ? AND status = 1 AND time >= ? AND time <= ?`,
      count: `SELECT COUNT(*) AS totalCount FROM recharge WHERE phone = ? AND status = 1 AND time >= ? AND time <= ?`,
    },
    Withdraw: {
      query: `SELECT id_order AS id, money, 'negative' AS type, 'Withdraw' AS name, status, time FROM withdraw WHERE phone = ? AND status = 1 AND time >= ? AND time <= ?`,
      count: `SELECT COUNT(*) AS totalCount FROM withdraw WHERE phone = ? AND status = 1 AND time >= ? AND time <= ?`,
    },
    Commissions: {
      query: `SELECT commission_id AS id, SUM(money) AS money, 'positive' AS type, 'Commission' AS name, status, time FROM commissions WHERE phone = ? AND time >= ? AND time <= ? GROUP BY time`,
    ...
    // },
    Salary: {
      query: `SELECT id, amount AS money, 'positive' AS type, CONCAT(type, ' Salary') AS name, status, time FROM salary WHERE phone = ? AND time >= ? AND time <= ?`,
...
const FindSalery = async () => {
    ...
    const [users] = await connection.execute('SELECT * FROM users where status=1');
    ...
    for (const user of users) {
        ...
        let activeuser = 0;
        // direct user
        const [f1Users] = await connection.query('SELECT `phone`, `code` FROM users WHERE `invite` = ?', [code]);
        if (f1Users.length >= 0) {
            for (const f1User of f1Users) {
                const f1Phone = f1User.phone;
                const [f1Recharge] = await connection.query('SELECT SUM(money) AS user_recharge FROM recharge WHERE `phone` = ? AND `status` = 1 AND Date(today) = ?', [f1Phone, DateCheck]);
                RechargeLimit = RechargeLimit + parseInt(f1Recharge[0].user_recharge, 10);
                const [userrecharge] = await connection.query('SELECT * FROM recharge WHERE `phone` = ? AND `status` = 1 AND Date(today) = ?', [f1Phone, DateCheck]);
                if (userrecharge.length > 0) {
                    activeuser = activeuser + 1;
                }
            }
            console.log('rech ' + RechargeLimit + ' Active ' + activeuser);
        }
        let AmountFinal = 0;
        if (RechargeLimit >= 15000000 && activeuser >= 700) {
            AmountFinal = 200000;
        } else if (RechargeLimit >= 10000000 && activeuser >= 600) {
            AmountFinal = 120000;
        } else if (RechargeLimit >= 5000000 && activeuser >= 500) {
            AmountFinal = 80000;
        } else if (RechargeLimit >= 2500000 && activeuser >= 400) {
            AmountFinal = 35000;
        } else if (RechargeLimit >= 1200000 && activeuser >= 300) {
            AmountFinal = 20000;
        } else if (RechargeLimit >= 300000 && activeuser >= 200) {
            AmountFinal = 12000;
        } else if (RechargeLimit >= 150000 && activeuser >= 100) {
            AmountFinal = 5000;
        } else if (RechargeLimit >= 70000 && activeuser >= 50) {
            AmountFinal = 3500;
        } else if (RechargeLimit >= 35000 && activeuser >= 25) {
            AmountFinal = 2000;
        } else if (RechargeLimit >= 9000 && activeuser >= 9) {
            AmountFinal = 900;
        } else if (RechargeLimit >= 6000 && activeuser >= 6) {
            AmountFinal = 600;
        } else if (RechargeLimit >= 3000 && activeuser >= 3) {
            AmountFinal = 300;
        }
        if (AmountFinal > 0) {
            const sql = `INSERT INTO transaction SET
                ...
                today = ?,
                time = ?`;
            await connection.execute(sql, ['Salery', phone, AmountFinal, 'credit', 0, checkTime, checkTime]);
            await connection.query('UPDATE users SET money = money+? WHERE phone = ?', [AmountFinal, phone]);
        }
...
//         monthamount = 30;
//     }
//     return { "level": level, "one": oneamount, "month": monthamount };
// };

// const MonthlyVipSetup = async (req, res) => {
//     function formateT(params) {
//         let result = (params < 10) ? "0" + params : params;
//         return result;
//     }
...
//                     today = ?,
//                     time = ?`;
//                 await connection.execute(sql, ['Monthly Vip Bonus', UserPhone, data.monthly_vip, 'credit', 1, data.vip_level, checkTime, checkTime]);
//                 await connection.query('UPDATE users SET money=money+? WHERE id_user = ?', [data.monthly_vip, UserId]);
//             }
//             // console.log(data.id_user,Rank);
//         });
//         res.status(200).json({
//             message: 'Success',
//             status: true,
...
// const FindVipSetup = async (req, res) => {
//     function formateT(params) {
//         let result = (params < 10) ? "0" + params : params;
//         return result;
//     }
...
//     let checkTime = timerJoin(time);
//     try {
//     console.log("vanshaj1")
//         const [user] = await connection.execute('SELECT * FROM users');
//         user.forEach(async (data) => {
//             let UserId = data.id_user;
//             let UserPhone = data.phone;
//             const [userBet] = await connection.query('SELECT SUM(money+fee) AS user_bet FROM minutes_1 WHERE phone = ?', [UserPhone]);
//             if (userBet[0].user_bet > 0) {
//                 let coin = userBet[0].user_bet;
//                 let Rank = FindRankByBetting(coin);
//                 // console.log('coin'+coin);
//                 if (data.vip_level < Rank.level) {
//                     console.log('userbet' + userBet[0].user_bet + ' user ' + UserPhone + ' Coin ' + coin);
//                     console.log('rank' + Rank.level);
...
//                     today = ?,
//                     time = ?`;
//                     await connection.execute(sql, ['Vip Level Bonus', UserPhone, Rank.one, 'credit', 1, checkTime, checkTime]);
//                     await connection.query('UPDATE users SET vip_level = ?,monthly_vip=?,money=money+? WHERE id_user = ?', [Rank.level, Rank.month, Rank.one, UserId]);
//                     console.log("True");
//                 }
//             }
//             // console.log(data.id_user,Rank);
